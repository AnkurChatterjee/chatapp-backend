...
const mockTestCases = require("./sendMessage.test.constants");
const app = require("../../../../app");

describe("Testing send message to a group without jwt token", () => {
...
describe("Testing send message to group by an user who is not group member or admin", () => {
  let accessToken;
...
describe("Testing send message to a group by a member", () => {
  let accessToken;
  //set up user token before test
  beforeAll(async () => {
    const loginResponse = await request(app)
